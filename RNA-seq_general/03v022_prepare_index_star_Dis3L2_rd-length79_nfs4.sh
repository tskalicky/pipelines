...
#PBS -l select=1:ncpus=5:mem=200gb:scratch_local=100gb
#PBS -j oe
#PBS -N STAR_index_dor_RSEM_aln_Dis3L2_RDlength79
#
...
GENOME="/storage/brno3-cerit/home/tskalicky/genomes/human/ensembl91/RSEM_index/star_align/GRCh38.91.dna.primary_assembly.idx.fa"
GTF="/storage/brno3-cerit/home/tskalicky/genomes/human/ensembl91/GRCh38.91_with_tRNA_modif_for_RSEM.sorted.gtf.gz"
...
GENOME_NAME="GRCh38.91.dna"
GTF_NAME=$(basename $GTF)

# unpigz -p $THREADS $GENOME_NAME
unpigz -p $THREADS $GTF_NAME

GENOME2="GRCh38.91.dna.primary_assembly.idx.fa"
GTF2=${GTF_NAME%.*}
GEN_DIR=${GENOME_NAME%.*.*}"_RD_LENGTH_79_for_RSEM_align"
...
# Using RSEM generated indexes for alignment requires STAR to use at least 150GB RAM for its own indexes
STAR --limitGenomeGenerateRAM 150026865706 --runMode genomeGenerate --runThreadN $THREADS --genomeDir $SCRATCHDIR/$GEN_DIR --genomeFastaFiles $SCRATCHDIR/$GENOME2 \
