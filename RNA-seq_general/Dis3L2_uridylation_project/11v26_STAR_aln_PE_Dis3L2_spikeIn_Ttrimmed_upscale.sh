...
#PBS -j oe
#PBS -N STAR_Dis3L2_SpikeIn_uridil_upscale
#
...
############################################################################################
### Variables
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/Dasa_spikein/uridylation/UPSCALE/trimT"
OUTPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/Dasa_spikein/uridylation/mapping_trimmedT/UPSCALE"
...
GENOME_INDEX="/storage/brno3-cerit/home/tskalicky/genomes/human/ensembl91/STAR_index/GRCh38_RD_LENGTH_100_STAR2.5.2" # Genome index with --sjdbOverhang 79 - Only for Dis3L2 RNA-seq. length=125
...
find "$INPUT_DIR" -maxdepth 1 -name "*good_pairs_R1.fastq.gz" -exec cp -vt "$SCRATCHDIR" {} + # this is not working at every occasion!!
find "$INPUT_DIR" -maxdepth 1 -name "*good_pairs_R2.fastq.gz" -exec cp -vt "$SCRATCHDIR" {} +
find "$INPUT_DIR" -maxdepth 1 -name "*all_singletons.fastq.gz" -exec cp -vt "$SCRATCHDIR" {} +
...
declare -a OAT_cyto=($(find . -maxdepth 1 -name 'trimT_DIS3l2_upscale_OAT_cyto*.fastq' -exec basename {} \; | sort -n))
declare -a OAT_nonfrac=($(find . -maxdepth 1 -name 'trimT_DIS3l2_upscale_OAT_nonfrac*.fastq' -exec basename {} \; | sort -n))
declare -a OAT_nucl=($(find . -maxdepth 1 -name 'trimT_DIS3l2_upscale_OAT_nucl*.fastq' -exec basename {} \; | sort -n))
declare -a U12_cyto=($(find . -maxdepth 1 -name 'trimT_DIS3l2_upscale_U12_cyto*.fastq' -exec basename {} \; | sort -n))
declare -a U12_nonfrac=($(find . -maxdepth 1 -name 'trimT_DIS3l2_upscale_U12_nonfrac*.fastq' -exec basename {} \; | sort -n))
declare -a U12_nucl=($(find . -maxdepth 1 -name 'trimT_DIS3l2_upscale_U12_nucl*.fastq' -exec basename {} \; | sort -n))
...
for (( w=0; w < ${quantity}; w += 3 ));
do
	SINGLETON="${ALL_ARRAYS[$w]}"
	SAMPLE_R1="${ALL_ARRAYS[$w+1]}"
	SAMPLE_R2="${ALL_ARRAYS[$w+2]}"
	if [[ -f $SAMPLE_R1 ]]; then
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Now I am processing PE reads $SAMPLE_R1 and $SAMPLE_R2 - alignment"
		...
		--outFileNamePrefix ${SAMPLE_R1%"_aligned_unmapped_"*}_unmapped.${GTF%.*} \
		...
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Done processing PE reads $SAMPLE_R1 and $SAMPLE_R2 - alignment"
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Now I am processing SINGLETON reads $SINGLETON - alignment"
		$STAR --runMode alignReads --runThreadN $THREADS --genomeDir $GEN_DIR \
		--readFilesIn $SINGLETON \
		--sjdbOverhang $RD_LENGTH --sjdbGTFfile $GTF \
		--outFileNamePrefix ${SINGLETON%"_aligned_unmapped_"*}_singletons.${GTF%.*} \
		--outFilterMultimapNmax 20 --alignSJoverhangMin 8 --alignSJDBoverhangMin 1 \
		--outFilterMismatchNmax $NO_MISMATCHES --outFilterMismatchNoverReadLmax $PER_MISMATCHES \
		--alignIntronMin 20 --alignIntronMax $MAX_INTRON --alignMatesGapMax 1000000 \
		--outFilterMatchNmin 0 --outFilterMatchNminOverLread 0.66 \
		...
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Done processing PE reads $SAMPLE_R1 and $SAMPLE_R2 - alignment"
	elif [[ -f $SINGLETON ]]; then
		echo "Your aray sorting and counter is faulty! You will not align the correct reads! Quiting..." && exit 1
	elif [[ -f SAMPLE_R2 ]]; then
		echo "Your aray sorting and counter is faulty! You will not align the correct reads! Quiting..." && exit 1
	else
		date +"%d/%m/%Y %H:%M:%S"
		echo "There is no $SAMPLE_R1 file for mapping!" && exit 1
	fi
done
wait
####################################################################################################
