...
#PBS -j oe
#PBS -N STAR_aln_Dis3L2_RNAseq_all
#
...
############################################################################################
### Variables
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/mapping/deduplicated_reads"
OUTPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/mapping/STAR_genome"
...
APPENDIX4=".dedupl.fq"
APPENDIX5=".dedupl.fq"
APPENDIX6="_1.dedupl.fq"
APPENDIX7="_2.dedupl.fq"
...
declare -a WT1=($(find . -maxdepth 1 -name '*_1_WT_flexbar*.fastq' -exec basename {} \; | sort -n))
declare -a WT2=($(find . -maxdepth 1 -name '*_2_WT_flexbar*.fastq' -exec basename {} \; | sort -n))
declare -a WT3=($(find . -maxdepth 1 -name '*_3_WT_flexbar*.fastq' -exec basename {} \; | sort -n))
declare -a KO1=($(find . -maxdepth 1 -name '*_1_KO_flexbar*.fastq' -exec basename {} \; | sort -n))
declare -a KO2=($(find . -maxdepth 1 -name '*_2_WT_flexbar*.fastq' -exec basename {} \; | sort -n))
declare -a KO3=($(find . -maxdepth 1 -name '*_3_WT_flexbar*.fastq' -exec basename {} \; | sort -n))
declare -a ARRAY_NAMES=("WT1" "WT2" "WT3" "KO1" "KO2" "KO3")
declare -a ALL_ARRAYS=("${WT1[@]}" "${WT2[@]}" "${WT3[@]}" "${KO1[@]}" "${KO2[@]}" "${KO3[@]}" )
...
quantity="${#ALL_ARRAYS[@]}"
#
echo "There are $WT1_num WT1 samples that will be mapped."
...
echo "There are $KO1_num KO1_num samples that will be mapped."
...
# rm -v $SCRATCHDIR/*.dedupl.fq # only when the deduplicated reads were created in previous run
