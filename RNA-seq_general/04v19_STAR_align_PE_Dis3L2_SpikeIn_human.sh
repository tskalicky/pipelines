...
#PBS -l select=1:ncpus=40:mem=1000gb:scratch_local=250gb
#PBS -j oe
#PBS -N STAR_aln_Dis3L2_PE_SpikeIn
#
...
############################################################################################
### Variables
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/Dasa_spikein/mapping/Scerevisiae_genome/bbmap/unmapped_reads"
OUTPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/Dasa_spikein/OAT_RNU12_mapping"
...
#
#
MY_RAM=400 # Max RAM memory for Samtools sort
THREADS=$PBS_NUM_PPN
...
find "$INPUT_DIR" -maxdepth 1 -name "*.fq" -exec cp -vt "$SCRATCHDIR" {} +
...
do
	...
	if [[ -f $SAMPLE ]]; then
		...
		--limitSjdbInsertNsj 10000000 --limitIObufferSize  1500000000 --outMultimapperOrder Random --outSAMtype BAM SortedByCoordinate & # --readFilesCommand zcat for reading .gz files; --outWigType bedGraph # --outSAMstrandField intronMotif - ENCODE uses this just for UNSTRANDED mapping
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
