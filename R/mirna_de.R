...
####################################################################################################
#
# INPUT_COUNTS - table with raw genes counts -> first column = miRBase miRNA id, first row = patient id
#
...
SAMPLES_DESC<-"/home/jan/Data/projects/smida_rna-seq/data/design.txt" # Samples description; please put reference (intercept) always first!

SAMPLES_REMOVE<-c("55_PDAC", "12_PDAC", "38_PDAC", "33_PDAC", "29_PDAC", "10_PDAC") # Sample to remove; can be empty
...
condsToCompare<-c("Ccyto", "11cyto") # In case we have intercepted comparison only second condition is selected; in case we have non-intercepted both conditions are extracted; intercept (if applicable and compared) should be always on the first position
...
REMOVE_COUNTS<-200000 # Remove samples bellow this read count; recommended is 500000 for most of the miRNA-seq, under certain conditions could be lowered down to 200000
...
APPENDIX<-"_kackaTest2" # Append something to the name of the output folder? Can be empty
name<-APPENDIX # Not using here, only in mRNA-Seq

####################################################################################################
# Function to check installed packages and installing them if they are missing for CRAN and Bioconductor packages
packages<-c("RColorBrewer", "gplots", "rgl", "pheatmap", "ggrepel", "ggplot2", "dplyr", "ggpubr", "magrittr", "dplyr") # CRAN packages
packages_bioc<-c("DESeq2", "edgeR", "vsn", "limma") # Bioconductor packages
...
colnames(mrcounts)<-gsub(pattern = "^X", replacement = "", colnames(mrcounts)) # Rename of columns if necessary
colnames(mrcounts) <- unlist(lapply(colnames(mrcounts), function(x) strsplit(x, "_R1_")[[1]][1])) # Assign sample names
colnames(mrcounts) <- unlist(lapply(colnames(mrcounts), function(x) strsplit(x, "_trim")[[1]][1])) # Assign sample names
####################################################################################################
...
for(i in colnames(mrcounts)){
  print(i)
  colnames(mrcounts)[colnames(mrcounts)==i]<-samples_desc[samples_desc$sample==i, "name"]
  rownames(coldata)[rownames(coldata)==i]<-samples_desc[samples_desc$sample==i, "name"]
}

SAMPLES_REMOVE<-c(SAMPLES_REMOVE, colnames(mrcounts[,colSums(mrcounts)<REMOVE_COUNTS])) # Remove samples specified at the beginning and by count cutoff
print(paste0("Samples ", colnames(mrcounts[,colSums(mrcounts)<REMOVE_COUNTS]), " had counts bellow ", REMOVE_COUNTS, ". Removing."))
mrcounts<-mrcounts[,colSums(mrcounts)>=REMOVE_COUNTS]

print(paste0("Removing samples", SAMPLES_REMOVE))
# Remove unwanted samples
if (length(SAMPLES_REMOVE) != 0){ 
	# Remove samples selected at the beginning
	mrcounts<-mrcounts[,!(colnames(mrcounts) %in% SAMPLES_REMOVE)]
...
####################################################################################################
dir.create(paste(OUTPUT_DIR, sep="/"), recursive = T)
setwd(paste(OUTPUT_DIR, sep="/"))
...
pdf(file="counts_barplot.pdf", width=10, height=8)
  ...
  abline(h = c(200000, 500000), col="red")
dev.off()

# Make a pieplot of miRNAs with >=5% expression; the rest is summarized to "other
pdf(file="expression_pie.pdf", width = 10, height = 6)
par(mfrow=c(2,2))
for(featureColumnNum in 1:(ncol(mrcounts))){
  featureColumnMain<-as.matrix(mrcounts[, featureColumnNum]) # Get one column from the input
  ...
  lbls<-rownames(mrcounts)[pctMain>=5] # Labels
  lbls<-c(lbls, "other")
  lbls<-paste(lbls, pct) 
  ...
  pie(featureColumn, labels = lbls, col=rainbow(length(lbls)), main=paste("Pie Chart of Gene Expression", 
      colnames(mrcounts)[featureColumnNum], sum(featureColumnMain), sep="\n"))
}
dev.off()
...
pdf(file="expression_barplot_top10.pdf", width = 10, height = 6)
  par(mfrow=c(1,2))
  par(mar = c(5,10,4,4) + 0.1) ## default is c(5,4,4,2) + 0.1
  for(featureColumnNum in 1:(ncol(mrcounts))){
    featureColumnMain<-as.data.frame(mrcounts[, featureColumnNum]) # Get one column from the input
    rownames(featureColumnMain)<-rownames(mrcounts)
    colnames(featureColumnMain)<-"counts"
    featureColumnMain$names<-rownames(featureColumnMain)
    featureColumnMain<-featureColumnMain %>% arrange(-counts)
    cntMain<-featureColumnMain$counts/100000 # Make labels with number per 100K reads
    
    # Merge features < 5% to others, otherwise to many lables 
    cnt<-cntMain[1:10] # Get abundant features
    cnt<-c(cnt, sum(cntMain[11:length(cntMain)])) # Other features - added together to one
    pct<-(cnt/sum(cnt))*100 # Percentages labeles
    lbls<-featureColumnMain$names[1:10] # Labels
    lbls<-c(lbls, "other")
    lbls<-paste(lbls, round(cnt, 2)) 
    lbls <- paste(lbls, "/100K", sep="") # ad % to labels     
    
    featureColumn<-featureColumnMain[1:10, "counts"] # Get subset of the main table for the abundant
    featureColumn<-c(featureColumn, sum(featureColumnMain[11:length(cntMain), "counts"])) # Get the others
    
    lbls<-gsub("/100K", "", lbls)
    xaxis<-barplot(featureColumn, col=rainbow(length(lbls)), width=2, xlim=c(0, max(featureColumn)*1.3), horiz=TRUE,
                   main=paste("Plot of Gene Expression for Top 10", colnames(mrcounts)[featureColumnNum], 
                              sum(featureColumnMain$counts), sep="\n"))
    axis(2, at=xaxis, labels=lbls, las=1)
    text(x = featureColumn, y = xaxis, label = paste(round(pct, 2), "%", " "), pos = 4, cex = 1, col = "red")
  }
dev.off()

# Write frequencies table
frequencies<-prop.table(as.matrix(mrcounts), margin=2)*100
...
pdf(file=paste("volcanoplot_", condsToCompare[1], "_vs_", condsToCompare[2],".pdf", sep=""))
	...
	text(res[RANGE,]$log2FoldChange, -log(res[RANGE,]$padj,10),
	     labels=rownames(res)[RANGE], cex=0.3, pos=3)
...
pdf(file=paste("MAplot_", condsToCompare[1], "_vs_", condsToCompare[2],".pdf", sep=""))
  ...
  text(res[1:TOP,]$baseMean, res[1:TOP,]$log2FoldChange, labels=rownames(res)[1:TOP], cex=0.3, pos=3)
...
resForPlot$Gene<-rownames(resForPlot)
...
pdf(file=paste("volcanoplot_", condsToCompare[1], "_vs_", condsToCompare[2], "_noIndFilt.pdf", sep=""))
	...
	text(resNoFil[RANGE,]$log2FoldChange, -log(resNoFil[RANGE,]$padj,10),
         labels=rownames(resNoFil)[RANGE], cex=0.3, pos=3)
	legend("bottomleft", condsToCompare[1], cex=0.5)
	legend("bottomright", condsToCompare[2], cex=0.5)
	abline(h=-log(P_THRESHOLD, 10), col="red", lty=2)
	abline(v=c(-LFC_THRESHOLD, LFC_THRESHOLD), col="darkblue", lty=2)
...
pdf(file=paste("MAplot_", condsToCompare[1], "_vs_", condsToCompare[2],"_noIndFilt.pdf", sep=""))
  ...
  DESeq2::plotMA(res, alpha=P_THRESHOLD, main=paste("MA plot ",condsToCompare[2], " vs ", condsToCompare[1], " top ", TOP, " genes", sep=""), 
                 ylim=c(-max(abs(res$log2FoldChange), na.rm=T), max(abs(res$log2FoldChange), na.rm=T)))
  abline(h=c(-LFC_THRESHOLD, LFC_THRESHOLD), col="darkblue", lty=2)
  text(resNoFil[1:TOP,]$baseMean, resNoFil[1:TOP,]$log2FoldChange, labels=rownames(res)[1:TOP], cex=0.3, pos=3)
  legend("bottomright", condsToCompare[1], cex=0.5)
  legend("topright", condsToCompare[2], cex=0.5)
dev.off()
...
resForPlot$Gene<-rownames(resNoFil)
results = mutate(resForPlot, sig=ifelse(resForPlot$padj<P_THRESHOLD, paste0("padj<", P_THRESHOLD), "Not Sig"))
p = ggplot(results, aes(log2FoldChange, -log10(padj))) +
  geom_point(aes(col=sig), size=0.5) +
  scale_color_manual(values=c("black", "red"))
...
for(i in 1:length(selectAll)){
  plotCounts(dds, gene=selectAll[i], intgroup="condition")
...
columnsTmp<-c("logCPM", "logFC", "PValue", "FDR", "LR", "tgw.Disp", "UpDown")
...
pdf(file=paste0("edgeR_MAplot_", condsToCompare[1], "_vs_", condsToCompare[2], ".pdf"))
  ...
  text(x=gene.labels$logCPM, y=gene.labels$log2FoldChange, labels=rownames(gene.labels), cex=0.7, pos=1)
dev.off()
