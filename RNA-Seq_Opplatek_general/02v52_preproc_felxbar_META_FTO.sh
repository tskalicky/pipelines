...
#PBS -l select=1:ncpus=21:mem=50gb:scratch_local=250gb
#PBS -j oe
#PBS -N 02v52_preprocess_SE_FTO
#
...
do
	...
	flexbar -t $TRIMDIR$SAMPLENAME"_qual_trim" -r $READ_FOR -q TAIL -qf i1.8 -qt 30 -m 17 -n 10 -z GZ & 
...
do
	READ_FOR2=$j
	echo "Now I am processing SE reads $READ_FOR2 - FastQC"
	echo "Now I am processing SE reads $READ_FOR2 - FastQC" >> $SCRATCHDIR/Pipeline_debuging.txt # debugging will be removed
	fastqc --outdir $QCDIR --format fastq $READ_FOR2 &

	echo "Done processing SE reads $READ_FOR2 - FastQC"
