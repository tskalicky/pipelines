...
#PBS -l select=1:ncpus=12:mem=200gb:scratch_local=250gb:os=debian9
#PBS -j oe
#PBS -N 02v42_fastQC_PE_FTO

#PBS -l walltime=336:0:0 
#PBS -q default@wagap-pro.cerit-sc.cz 
#PBS -l select=1:ncpus=25:mem=50gb:scratch_local=300gb:os=debian9:host=zewura7.cerit-sc.cz
#
...
### Variables
set -e
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/FTO/trimmed"
OUTPUT_DIR="/storage/brno3-cerit/home/tskalicky/FTO/trimmed/quality_check"
#
...
cd $SCRATCHDIR
ls -l $SCRATCHDIR >> $SCRATCHDIR/Pipeline_debuging.txt # debugging will be removed
...
#
echo "Now I am processing PE reads - Fastqc check" >> $SCRATCHDIR/Pipeline_debuging.txt # debugging will be removed
...
cd $QCDIR
echo "entering" $QCDIR
echo "entering" $QCDIR >> $SCRATCHDIR/Pipeline_debuging.txt # debugging will be removed
...
date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
echo "Script finished on:" >> $SCRATCHDIR/Pipeline_debuging.txt # debugging will be removed
date +"%d/%m/%Y %H:%M:%S $HOSTNAME" >> $SCRATCHDIR/Pipeline_debuging.txt # debugging will be removed