...
#PBS -l select=1:ncpus=2:mem=100gb:scratch_local=150gb
#PBS -j oe
#PBS -N 05v41_METTL16_PAR1-2_Collapsing_PCR_duplicates
#
...
############################################################################################
### Variables
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/METTL16/mapping/deduplicated/METTL16_PAR1-2/results/alignment/genome"
...
OUTPUT_DIR="/storage/brno3-cerit/home/tskalicky/METTL16/mapping/deduplicated/METTL16_PAR1-2/genome/PCR_collapsed"

MY_RAM=100 # Max RAM memory for Samtools sort
THREADS=$PBS_NUM_PPN
SAMPLES="METTL16_PAR1-2"
APPENDIX=".out.bam"
APPENDIX2=".md.sam"
APPENDIX3=".tag.bed"
...
do
	...
	echo "Now I am getting genomic distribution of CLIP tags for sample $READ_FOR3"
	perl $BED2ANNOTATION -dbkey hg19 -ss -big -region -v -summary $SAMPLENAME3".tag.uniq.annot.summary.txt" \
	$SAMPLENAME3".tag.uniq.bed" $SAMPLENAME3".tag.uniq.annot.txt"
...
do
	...
	echo "Now I am getting the length distribution of unique tags for sample:" $READ_FOR4
	awk '{print $3-$2}' $READ_FOR4 | sort -n | uniq -c | awk '{print $2"\t"$1}' > $SAMPLENAME5".uniq.len.dist.txt"
	echo "Done getting the length distribution of unique tags for sample:" $READ_FOR4
