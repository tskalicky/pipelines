...
#PBS -l select=1:ncpus=2:mem=100gb:scratch_local=100gb
#PBS -j oe
#PBS -N 12_09_intersect_annot_all
...
BAMCOMPARE="/storage/brno3-cerit/home/tskalicky/ABH8/bamCompare"
...
cp -av $PEAKS $SCRATCHDIR
cp -av $BAMCOMPARE/*.FW.bed $SCRATCHDIR
cp -av $BAMCOMPARE/*.RV.bed $SCRATCHDIR
...
#
date +"%d/%m/%Y %H:%M:%S"
echo "Sorting and merging shuffled file ABH8_CLIP_vs_RNAseq_bamCompare_bin10_SES_log2_and_subtract_nosmooth.shuf.bed"
sortBed -i ABH8_CLIP_vs_RNAseq_bamCompare_bin10_SES_log2_and_subtract_nosmooth.shuf.bed | \
mergeBed -s -c 4,5,6,7 -o median,median,distinct,median -i stdin | \
awk -F'[\t]' '{print $1 "\t" $2 "\t" $3 "\t" $5 "\t" $6 "\t" $7 "\t" $8 }' > ABH8_CLIP_vs_RNAseq_bamCompare_bin10_SES_log2_and_subtract_nosmooth.shuf.sm.bed
date +"%d/%m/%Y %H:%M:%S"
echo "Finnished Sorting and merging shuffled file ABH8_CLIP_vs_RNAseq_bamCompare_bin10_SES_log2_and_subtract_nosmooth.shuf.bed"
#
#
intersectBed -wo -s -f 0.5 -split \
-a "ABH8_CLIP_vs_RNAseq_bamCompare_bin10_SES_log2_and_subtract_nosmooth.shuf.sm.bed" \
-b "PIPE-CLIP_ABH8_CLIP_enrichedClusters_n50.bed" \
> Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.bed
#
intersectBed -v -split \
-a "ABH8_CLIP_vs_RNAseq_bamCompare_bin10_SES_log2_and_subtract_nosmooth.shuf.sm.bed" \
-b "PIPE-CLIP_ABH8_enrichedClusters_n50.bed" \
> NOT_Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.bed
#
date +"%d/%m/%Y %H:%M:%S"
echo "Shufling, Sorting and merging shuffled file Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.bed"
awk -F'[\t]' -v OFS='\t' '{print $1, $2, $3, $11, $5, $6, $7 }' Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.bed \
> Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.shuf.bed
#
sortBed -i Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.shuf.bed | \
mergeBed -s -c 4,5,6,7 -o distinct,median,distinct,median -i stdin \
> Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.shuf.sm.bed
awk -F'[\t]' -v OFS='\t' '{print $1, $2, $3, $5, $6, $7, $8 }' Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.shuf.sm.bed \
> Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.shuf2.sm.bed
date +"%d/%m/%Y %H:%M:%S"
echo "Finnished shufling, sorting and merging shuffled file Intersect_ABH8_bamcompared_and_PIPE-CLIP_enrichedClusters_n50.bed"
#
# wait
wait
############################################################################################
