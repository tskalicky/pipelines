...
#PBS -l select=1:ncpus=1:mem=100gb:scratch_local=150gb
#PBS -j oe
#PBS -N 05v42_zkouska_METTL16_PAR1-2_Collapsing_PCR_duplicates
#
...
############################################################################################
### Variables
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/METTL16/mapping/deduplicated/METTL16_PAR1-2/genome/PCR_collapsed/failed_job_614651.wagap-pro.cerit-sc.cz/BED"
...
cp -av $MY_GENOME $SCRATCHDIR
cp -av $INPUT_DIR/*.md.tag.bed $INPUT_DIR/*.md.mutation.txt $SCRATCHDIR/BED
cd $SCRATCHDIR
...
if [[ $quantity -gt 1 ]]; then
	...
	do
		...
		echo "Now I am getting the length distribution of unique tags for sample:" $d
