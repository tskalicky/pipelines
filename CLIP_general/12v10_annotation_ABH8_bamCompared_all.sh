...
#PBS -j oe
#PBS -N 12v08_annotation_ABH8_bamcompared_all
#
...
# Also be aware that when merging using mergeBed -s option for considering strand info, the program will insert strand info as an extra 
# field on 4th position and the others will thus get renumbered!
#
############################################################################################
...
do
	...
	do
		...
		awk -F'[\t]' -v OFS='\t' '{print $1, $2, $3, $11, $5, $6, $7, $4}' "$SCRATCHDIR/$SAMPLENAME/$SAMPLE3" > "$REN_MERGED/$SAMPLENAME3".ren.bed
		date +"%d/%m/%Y %H:%M:%S"
		...
		mergeBed -s -c 4,5,6,7,8 -o distinct,median,distinct,median,distinct -i stdin > "$REN_MERGED/$SAMPLENAME3".rsm.bed
		awk -F'[\t]' -v OFS='\t' '{print $1, $2, $3, $5, $6, $7, $8, $9 }' > "$REN_MERGED/$SAMPLENAME3".rsm.shuf3.bed
		# sortBed -i temp_2_tRNA.ren.bed | \
		...
		# awk -F'[\t]' '{print $1 "\t" $2 "\t" $3 "\t" $5 "\t" $6 "," $8 "," $9 "\t" $7 }' temp_2_tRNA.rsm.bed > temp_2_tRNA.rsm.shuf4.bed
		date +"%d/%m/%Y %H:%M:%S"
