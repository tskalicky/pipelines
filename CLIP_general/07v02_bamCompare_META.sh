...
#PBS -l select=1:ncpus=9:mem=100gb:scratch_local=50gb
#PBS -j oe
...
--verbose \
-of bedgraph \
-o ABH8_CLIP_vs_RNAseq_enrichment_compare_RPKM_nosmooth.bed
date +"%d/%m/%Y %H:%M:%S" 
echo "bamCompare is finnished"

############################################################################################
### Copy data from scratch back to home dir and clean scratch
mkdir -p $OUTPUT_DIR
rm $SCRATCHDIR/*.sorted.bam
