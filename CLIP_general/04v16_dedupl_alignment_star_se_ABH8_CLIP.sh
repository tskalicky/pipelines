...
#PBS -l select=1:ncpus=20:mem=100gb:scratch_local=250gb:os=debian9
#PBS -j oe
#PBS -N 04v15_alignment_se_ABH8_CLIP
#
...
INPUT_DIR=/storage/brno3-cerit/home/tskalicky/ABH8/preprocessed/data/trimmed

OUTPUT_DIR=/storage/brno3-cerit/home/tskalicky/ABH8/mapping/deduplicated
OUTPUT_DIR=${OUTPUT_DIR}/results
OUTPUT_DIR_QC=${OUTPUT_DIR}/qc

MY_RAM=100 # Max RAM memory for Samtools sort
THREADS=$PBS_NUM_PPN
...
APPENDIX="_CLIPseq_flexbar.fastq.gz"
APPENDIX2="_flexbar.dedupl.fastq"
APPENDIX3="_flexbar.fastq"
APPENDIX4=".dedupl.fastq.gz"
...
GENOME_INDEX="/storage/brno3-cerit/home/tskalicky/genomes/human/ensembl91/STAR_index/GRCh38.dna.primary_assembly_RD_LENGTH_49" # Genome index with --sjdbOverhang 124 - Only for FTO CLIP and INPUT with lib. length=50
MY_GENOME="/storage/brno3-cerit/home/tskalicky/genomes/human/ensembl91/GRCh38.dna.primary_assembly.fa.gz"
...
do
	...
	# It is good to check the number of reads after collapsing
	wc -l $SAMPLENAME".dedupl.fastq" > $SAMPLENAME".dedupl.summary.txt"
done
#wait
wait
...
do
	READ_LIB3=$x
	echo "Now I am compressing SE reads $READ_LIB3"
	pigz -v -p $THREADS $READ_LIB3
	echo "Done compressing SE reads $READ_LIB3"
done
# wait
wait
rm $SCRATCHDIR/*$APPENDIX3 # deleting original data
#####################################################################################################
...
## works only with sorted BAM and Pair-End data
...
####################################################################################################
### Finalize and copy results from scratch back to home dir and clean scratch
mkdir -p $OUTPUT_DIR
