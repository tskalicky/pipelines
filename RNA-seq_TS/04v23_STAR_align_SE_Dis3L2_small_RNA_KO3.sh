...
#PBS -j oe
#PBS -N STAR_aln_Dis3L2_smallRNA_KO3
#
...
############################################################################################
### Variables
INPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/Nandan_small_RNA_new_set/data/trimmed"
OUTPUT_DIR="/storage/brno3-cerit/home/tskalicky/Dis3L2/Nandan_small_RNA_new_set/mapping/KO3/STAR_genome"
...
cp -av $INPUT_DIR/DIS3l2_KO3_Nandan_small-RNA_flexbar.fastq.gz $SCRATCHDIR
...
declare -a LIBRARIES=($(find . -maxdepth 1 -name '*_flexbar.fastq' -exec basename {} \; | sort -n))
...
do
	...
	if [[ -f $SAMPLE ]]; then
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Now I am processing SE reads $SAMPLE - alignment"
		$STAR --runMode alignReads --runThreadN $THREADS_LIMIT --genomeDir $GEN_DIR \
		--readFilesIn $SAMPLE \
		...
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Done processing SE reads $SAMPLE - alignment"
	else
		date +"%d/%m/%Y %H:%M:%S"
...
rm $SCRATCHDIR/*.fastq # remove original trimmed reads
