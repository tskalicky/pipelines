...
#PBS -l mem=250gb
#PBS -k oe
#PBS -N bbmap_Dis3L2_spikein_upscale_transcripts
...
############################################################################################
### Variables
INPUT_DIR="/home/users/tskalicky/CEITEC/Dis3L2/Dasa_SpikeIn/NEW_upscale/data/trimmed"
MY_GENOME="/home/users/tskalicky/CEITEC/genomes/human/OAT_RNU12_transcripts/OAT_RNU12_isoforms.fa"
MY_GTF="/home/users/tskalicky/CEITEC/genomes/human/OAT_U12_mapping.gtf.gz"
OUTPUT_DIR="/home/users/tskalicky/CEITEC/Dis3L2/Dasa_SpikeIn/mapping/OAT_U12/bbmap_transcriptome/upscale"
...
THREADS=$PBS_NUM_PPN
# Set MAX ram
MY_RAM="250" # SET THE AMOUNT OF RAM HERE !!!
...
declare -a OAT_CYTO=($(find . -maxdepth 1 -name  '*OAT_cyto*.fastq.gz' -exec basename {} \; | sort -n))
declare -a OAT_NONFRAC=($(find . -maxdepth 1 -name  '*OAT_nonfrac*.fastq.gz' -exec basename {} \; | sort -n))
declare -a OAT_NUCL=($(find . -maxdepth 1 -name  '*OAT_nucl*.fastq.gz' -exec basename {} \; | sort -n))
declare -a U12_CYTO=($(find . -maxdepth 1 -name  '*U12_cyto*.fastq.gz' -exec basename {} \; | sort -n))
declare -a U12_NONFRAC=($(find . -maxdepth 1 -name  '*U12_nonfrac*.fastq.gz' -exec basename {} \; | sort -n))
declare -a U12_NUCL=($(find . -maxdepth 1 -name  '*U12_nucl*.fastq.gz' -exec basename {} \; | sort -n))
