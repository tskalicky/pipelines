...
OUTPUT_DIR="/home/tomas/Bioinformatics/CEITEC/Dis3L2/Dasa_spikein/uridylation/NEW"
...
#
# Extract uniquelly mapped and unmapped reads from BAM files
for a in *$APPENDIX1
do
	SAMPLE=$a
	SAMPLENAME=${a%.*}
...
do
	...
	if [[ $SAMPLE2 = DIS3l2_*_unmapped.fastq ]] ; then
		...
		echo "Start extracting uridylated reads from FASTA file $SAMPLENAME2.fa"
...
do
	...
	$BBSPLITPAIRS  in=$SAMPLE3 out="$SAMPLENAME3"_good_pairs.fastq \
	outsingle="$SAMPLENAME3"_singletons.fastq minlen=17
	$REPAIR in="$SAMPLENAME3"_good_pairs.fastq out="$SAMPLENAME3"_fixed_good_pairs_R1.fastq \
	out2="$SAMPLENAME3"_fixed_good_pairs_R2.fastq outs="$SAMPLENAME3"_singletons2.fastq repair
