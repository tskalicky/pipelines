...
#PBS -k oe
#PBS -N star_Dis3L2_SpikeIn_OAT_U12_upscale
...
OUTPUT_DIR="/home/users/tskalicky/CEITEC/Dis3L2/Dasa_SpikeIn/mapping/OAT_U12/STAR/upscale"
...
gzip -dv $GENOME_NAME
gzip -dv $GTF_NAME
find "$OUTPUT_DIR" -maxdepth 1 -name "*.fastq.gz" -exec gzip -dv {} + 
#
...
do
	...
	if [[ -f $SAMPLE ]]; then
		...
		#
		date +"%d/%m/%Y %H:%M:%S $HOSTNAME"
		echo "Now I am processing PE reads $SAMPLE3 and $SAMPLE4 - alignment"
		# --limitBAMsortRAM 40000000000 has to be manually set because of custom small "genome"
		$STAR --runMode alignReads --runThreadN $THREADS --genomeDir $OUTPUT_DIR/$GEN_DIR \
		--genomeLoad NoSharedMemory --limitBAMsortRAM 40000000000 --readFilesIn $SAMPLE3 $SAMPLE4 \
		--sjdbOverhang $RD_LENGTH --sjdbGTFfile $GTF \
		--outFileNamePrefix ${SAMPLE3%"_flexbar_"*}.${GTF%.*} \
		--outFilterMultimapNmax 20 --alignSJoverhangMin 8 --alignSJDBoverhangMin 1 \
		--outFilterMismatchNmax $NO_MISMATCHES --outFilterMismatchNoverReadLmax $PER_MISMATCHES \
		--alignIntronMin 20 --alignIntronMax $MAX_INTRON --alignMatesGapMax 1000000 \
		--outFilterMatchNmin 0 --outFilterMatchNminOverLread 0.66 \
...
rm -v $OUTPUT_DIR/*.fastq # only when the deduplicated reads were created in previous run
